...
public class DrivePathRoutine extends TimeoutRoutineBase {
  ...
  public DrivePathRoutine(WantedTrajectory trajectory) {
    this(trajectory, 1.15);
  }

  public DrivePathRoutine(WantedTrajectory trajectory, double mult_constant) {
  ...
  public void update(@ReadOnly Commands commands, @ReadOnly RobotState state) {
    ...
    if (targeting.equals(FieldState.PieceType.CENTERLINE)) {
      ...
      commands.yCorrection =
          state.state.center_observations[targetN - 1].location().getY() + offset;
  ...
  public void onTimeout() {
    state.swerveWanted = Swerve.State.NEUTRAL;
  }
}
