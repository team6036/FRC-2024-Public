...
/** Wpilib HolomonicDriveController but it can also accept Pathplanner stuff */
public class PeninsulaHolonomicDriveController {
   ...
   */
  public PeninsulaHolonomicDriveController(
  ...
  PIDController d = new PIDController(15, 0, 1.5);
  //  PIDController d = new PIDController(15, 0, 1);

  {
    d.enableContinuousInput(-Math.PI, Math.PI);
  }
  ...
  public ChassisSpeeds calculate(
    ...
    double xMultiplier = Robot.onBlueAlliance ? 1.0 : -1.0;
    double xFF =
        xMultiplier * desiredLinearVelocityMetersPerSecond * trajectoryPose.getRotation().getCos();
    double yFF = desiredLinearVelocityMetersPerSecond * trajectoryPose.getRotation().getSin();

    //    double thetaFF =
    //        d.calculate(currentPose.getRotation().getRadians(), desiredHeading.getRadians());

    //        double errorBound = (Math.PI - (-Math.PI)) / 2.0;
    //        double pos_error =
    //            MathUtil.inputModulus(
    //                desiredHeading.getRadians() - currentPose.getRotation().getRadians(),
    //                -errorBound,
    //                errorBound);

    //        thetaFF += pos_error / 0.02;

    double thetaFF =
        m_thetaController.calculate(
            currentPose.getRotation().getRadians(), desiredHeading.getRadians());

    SmartDashboard.putNumber("Auto/thetaFF", thetaFF);
  ...
  public ChassisSpeeds calculate(
      Pose2d currenPose, PathPlannerTrajectory.State desiredState, Rotation2d desiredHeading) {
    return calculate(
        currenPose,
